...
type store = {
  ...
  reset: () => void;
  saveToDatabase: (images?: ImageData[]) => Promise<{ success: boolean; data?: any; error?: string }>;
  saveWithPhotos: (photos: { uri: string; type: string; orientation: string }[]) => Promise<{ success: boolean; data?: any; error?: string }>;
  loading: boolean;
  error: string | null;
};
...
export const useNutrientsStore = create<Nutrients & store>((set, get) => ({
  ...
  saveWithPhotos: async (photos: { uri: string; type: string; orientation: string }[]) => {
    const { carbs, protein, sodium } = get();
...
type RecommStore = {
  ...
  calculateRecommendations: (userProfile: { weight: number; height: number; age: number; gender: string }) => void;
...
export const useRecommStore = create<Recommendation & RecommStore>((set, get) => ({
  minCarb: 0,
  maxCarb: 0,
  minProtein: 0,
  maxProtein: 0,
  ...
  calculateRecommendations: (userProfile) => {
    ...
    const minProteinCalories = dailyCalories * 0.10;
